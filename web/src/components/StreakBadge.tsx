...
export default function StreakBadge({ current, longest }: StreakBadgeProps) {
  ...
  return (
    <div
      className={`p-4 bg-zinc-900/80 border border-zinc-800/60 rounded-lg flex items-center gap-3 transition-all duration-200 ${active ? 'border-l-[3px] border-l-amber-500' : ''}`}
    >
      <Flame
      ...
      />
      <div>
        <p className="text-[10px] text-zinc-500 uppercase tracking-widest mb-0.5 font-medium">
          Streak
        </p>
        <p
          className={`text-3xl font-bold font-mono tabular-nums leading-none ${active ? 'text-amber-400' : 'text-zinc-400'}`}
        >
          {current}
