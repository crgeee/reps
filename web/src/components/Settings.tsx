...
const SECTION_ICONS: Record<string, JSX.Element> = {
  ...
  device: <path strokeLinecap="round" strokeLinejoin="round" d="M9 17.25v1.007a3 3 0 01-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0115 18.257V17.25m6-12V15a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 15V5.25A2.25 2.25 0 015.25 3h13.5A2.25 2.25 0 0121 5.25z" />,
...
function SectionHeader({ icon, title }: { icon: string; title: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <svg className={`w-5 h-5 ${isAmber ? 'text-amber-500' : 'text-zinc-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        {SECTION_ICONS[icon]}
      </svg>
...
function ToggleRow({ label, description, checked, onChange }: {
  ...
  return (
    <label className="flex items-center justify-between cursor-pointer group">
      ...
      >
        <div className={`absolute top-1 w-4 h-4 rounded-full transition-all ${checked ? 'left-5 bg-zinc-900' : 'left-1 bg-zinc-400'}`} />
      </div>
    </label>
  );
}

function ListRow({ children, onRemove, removeLabel = 'Remove' }: {
