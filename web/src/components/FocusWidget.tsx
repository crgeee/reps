...
import { Timer, X, ChevronDown, Music, ExternalLink, Volume2, VolumeX } from 'lucide-react';
...
export default function FocusWidget() {
  ...
  useEffect(() => {
    localStorage.setItem('reps_focus_sound', JSON.stringify(soundEnabled));
  }, [soundEnabled]);
  ...
  return (
    <div className="sticky bottom-4 z-50 ml-auto mr-4 w-fit">
      {/* Timer always mounted so state persists across collapse */}
      <div className={expanded ? '' : 'hidden'}>
        <div className="anim-slide-up w-80 bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl overflow-hidden">
          {/* Header */}
          ...
          {/* Timer */}
          <div className="p-4">
            <FocusTimer soundEnabled={soundEnabled} />
          </div>
      ...
      {!expanded && (
        <button
          ...
          className="anim-scale-in flex items-center gap-2 px-3 py-2 bg-zinc-900 border border-zinc-700 rounded-full shadow-lg hover:bg-zinc-800 hover:border-zinc-600 hover:shadow-xl hover:scale-105 active:scale-95 transition-all duration-200 group"
        >
          <Timer className="w-4 h-4 text-amber-400" />
          {timerDisplay ? (
