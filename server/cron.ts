...
async function getNotifiableUsers(field: "notify_daily" | "notify_weekly"): Promise<NotifiableUser[]> {
...
export function startCronJobs(): void {
  ...
  cron.schedule("0 3 * * *", async () => {
    try {
      ...
      console.log(`[cron] Cleaned ${sessionCount} expired sessions, ${deviceCount} expired device codes.`);
    } catch (err) {
      console.error("[cron] Session cleanup failed:", err);
    }
  });

  console.log("[cron] Scheduled: daily briefing + digest (8:00 AM), weekly insight (Sun 8:00 PM), session cleanup (3:00 AM)");
}
